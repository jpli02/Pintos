...
process_execute (const char *cmd_line) 
{
  ...
  //printf ("The cmd_line is %s\n", cmd_line);

  /* Get file_name from the cmd_line */
  char *file_name;
  char *temp = NULL;
  file_name = malloc (strlen(fn_copy) + 1);
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  /* Deal with some thread info */
  thread_current ()->waiting_on=c->tid;
  /*if thread not waited before*/
  if (!c->used)
    sema_down (&thread_current ()->child_lock);
  
  
  int rtr_val=c->exit_status;
...
process_exit (void)
{
  ...
  if (thread_current ()->exit_status==-200)
    syscall_exit (-1);
  ...
  // printf ("the process unblocked is %d\n", cur->waited_thread->tid)
}
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* Transfer the file_name to argc and argv */
  char *token, *save_ptr;
  char *argv[128];
  int argc = 0;
  argv[argc++] = strtok_r((char *)file_name, " ", &save_ptr);
  ...
  if (kpage != NULL)
    {
      ...
      if (success)
      {
        *esp = PHYS_BASE;
        ...
        /*the gap we need to fix to be aligned*/
        if ((int)*esp % 4)
          {
            int fix_aligned = (int)*esp % 4 + 4;
            *esp -= fix_aligned;
            memcpy (*esp, &zero, fix_aligned);
          }
        ...
        *esp -= 4;
        ...
        int *address_of_argv = *esp;
        *esp -= 4;
        memcpy (*esp,&address_of_argv,4);
        *esp -= 4;
        memcpy (*esp,&argc,4);
